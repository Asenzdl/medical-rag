...
def get_tokenizer(model_dir: str):
    """缓存 tokenizer，避免重复加载"""
    logger.debug(f"加载 Tokenizer: {model_dir}")
...
def create_bert_engine(
    ...
    if backend == "tensorrt":
        from src.bert.tensorrt_engine import TensorRTEngine
        return TensorRTEngine(model_dir, tokenizer, label_map=label_map)
    else:
        from src.bert.onnx_engine import OnnxRuntimeEngine
        return OnnxRuntimeEngine(model_dir, tokenizer, provider, label_map=label_map)
...
if __name__ == "__main__":
    ...
    # 测试 ONNX Runtime CPU
    print("\n--- ONNX Runtime CPU ---")
    engine_cpu = create_bert_engine(MODEL_DIR, TOKENIZER_DIR, backend="cpu", provider="cpu")
